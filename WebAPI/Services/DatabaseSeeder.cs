...
namespace WebAPI.Services
{
    public class DatabaseSeeder
    {
        public static void Seed(ApplicationDbContext context)
        {
            ...
            {
                // Get valid RoleIds
                var roleIds = context.Roles.Select(r => r.RoleId).Take(20).ToList();
                for (int i = 1; i <= 20; i++)
                {
            ...
            {
                var orderIds = context.Orders.Select(o => o.OrderId).Take(20).ToList();
                var itemMasterIds = context.ItemMasters.Select(im => im.ItemId).Take(20).ToList();
            ...
            {
                var orderItemIds = context.OrderItems.Select(oi => oi.OrderItemId).Take(20).ToList();
            ...
            {
                var printingIds = context.Printings.Select(p => p.PrintingId).Take(20).ToList();
            ...
            {
                var billingIds = context.Billings.Select(b => b.BillId).Take(20).ToList();
                var dispatchIds = context.Dispatches.Select(d => d.DispatchId).Take(20).ToList();
